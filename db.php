...
function getConn()
{
    ...
    try {
        ...
        $conn = new PDO("mysql:host=" . DB_HOST . ";dbname=" . DB_NAME . "", DB_USERNAME, DB_PASSWORD, $opt);
...
function getTrip($tripId)
{
    try {
        ...
        //   fileLog("retrieved: " . var_export($trip, true));
        return $trip;
    } catch (PDOException $e) {
...
{
    try {
        $stmt = getConn()->prepare('INSERT INTO bookings(`trip_id`,`number_adults`,`number_children`,`total_cost`,`user_id`,`notes`,`booking_code`)
        VALUES (?,?,?,?,?,?,?)');
        $stmt->execute([$tripid, $adult, $children,  $cost, $userid,  $notes, $bookingCode]);
